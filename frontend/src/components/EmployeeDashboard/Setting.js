...
const Setting = () => {
        ...
        if (setting.newPassword !== setting.confirmPassword) {
            ...
            try {
                const response = await axios.put(
                    "https://projectserver-api.vercel.app/api/setting/change-password",
                    setting,
                    {
                        headers: {
    ...
    return (
        <div className='max-w-3x1 mx-auto mt-10 bg-white p-8 rounded-md shadow-md w-96'>
            <h2 className='text-2x1 font-bold mb-6'>Change Password</h2>
            <p className='text-red-500'>{error}</p>
            <form onSubmit={handleSubmit}>
                <div>
                    <label className='text-sm font-medium text-gray-700'>Old Password</label>
                    <input type='password' name='oldPassword' placeholder='Change Password' onChange={handleChange} className='mt-1 w-full p-2 border border-gray-300 rounded-md' required />
                </div>
                <div>
